...
var weekly4 = [{date: "0", content: "0"}];
if(sessionStorage.weekly4) {
  weekly4 = JSON.parse(sessionStorage.weekly4);
}
...
function clickOk() {
  ...
  if(inputDate == "MON") {
    ...
    weekly4.push({date: inputDate, content: inputContent});
  }
  if(inputDate == "TUE") {
  ...
  sessionStorage.weekly4 = JSON.stringify(weekly4);
}
...
window.onload = function() {
  if(sessionStorage.weekly4) {
    var weekly4 = JSON.parse(sessionStorage.weekly4);
    for(var i=0; i < weekly4.length; i++) {
      var date1 = weekly4[i].date;
      var content1 = weekly4[i].content;
