...
var weekly5 = [{date: "0", content: "0"}];
if(sessionStorage.weekly5) {
  weekly5 = JSON.parse(sessionStorage.weekly5);
}
...
function clickOk() {
  ...
  if(inputDate == "MON") {
    ...
    weekly5.push({date: inputDate, content: inputContent});
  }
  if(inputDate == "TUE") {
  ...
  sessionStorage.weekly5 = JSON.stringify(weekly5);
}
...
window.onload = function() {
  if(sessionStorage.weekly5) {
    var weekly5 = JSON.parse(sessionStorage.weekly5);
    for(var i=0; i < weekly5.length; i++) {
      var date1 = weekly5[i].date;
      var content1 = weekly5[i].content;
