...
function click1() { // when click the each number button
  ...
  display.value = input; // showing input
}
...
function clickEnter() { // when click the enter button
  if(input != accountNumber) // if the input is different with account number
  {
    alert("That account number does not exist!!"); // showing alert
  }
  else
  {
    location.href = link; // else -> go to next page(password page)
  }
}

function clickDel() { // if click del button
